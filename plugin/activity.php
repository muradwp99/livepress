...
add_action(
	'admin_menu',
	function () {
		add_submenu_page( 'livepress', 'Activity', 'Activity', 'edit_pages', 'livepress-activity', 'livepress_render_activity' );
	},
	21
);
...
function livepress_render_activity() {
	...
	livepress_screen_open(
		'Activity',
		sprintf(
			'The %d most recent changes across every page and article. Field edits name the person who made them; a plain save records only that WordPress wrote the document, because it does not keep an editor for that.',
	...
	if ( ! $rows ) {
		livepress_empty_state(
			...
			'Field edits and document saves will appear here as they happen, newest first.'
		);
	...
	foreach ( $rows as $r ) {
		...
		if ( 'field' === $r['what'] ) {
			$change = '<span class="lp-pill lp-pill--accent">edited</span>';
			$detail = sprintf(
			...
			$change = '<span class="lp-pill lp-pill--quiet">saved</span>';
			$detail = '<span class="lp-muted">Document written</span>';
		}
		...
		printf(
			...
			. '</tr>',
			esc_html( $r['time'] ? date_i18n( 'j M Y H:i', $r['time'] ) : 'unknown' ),
			esc_html( $r['time'] ? human_time_diff( $r['time'] ) . ' ago' : '' ),
			esc_url( (string) $edit ),
			esc_html( $doc->post_title ? $doc->post_title : $doc->post_name ),
			esc_html( $doc->post_type ),
			$change,
			$user ? esc_html( $user->display_name ) : '<span class="lp-muted">not recorded</span>',
			$detail
		);
	}
