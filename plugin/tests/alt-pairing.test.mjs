 ...
 *
 * Two conventions are live at once and a wrong answer is invisible: a
 * top-level picture is `cta_img` beside `cta_img_alt`, a repeater column is
 * `src` beside plain `alt`. Resolve the wrong one and the editor either
...
console.log("alt pairing: all assertions passed (both conventions, and no-alt returns null)");
